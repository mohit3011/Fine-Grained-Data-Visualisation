...
Botman.LandLayer.prototype.compute_surface_points = function() {
  ...
  var c = [];
	...
	if (file.type.match(textType)) {
		...
		//alert("maxix"+maxix);
		//alert("maxiy"+maxiy);
		maxix=Math.floor(maxix/10);
		maxiy=Math.floor(maxiy/10);
		//alert("maxix"+maxix);
		//alert("maxiy"+maxiy);
		var points=[];
		for(var i=0;i<12;i++)
		...
		for(var i=0;i<c.length;i++)
		{
			...
			var temp1=x[0]/maxix;
			var temp2=x[1]/maxiy;
			//alert("yoyo" + Math.floor(temp1));
			//alert(temp2);
			points[Math.floor(temp1)][Math.floor(temp2)]++;
		}
		//alert("points is "+ points);
